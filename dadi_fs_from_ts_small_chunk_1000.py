...
test_data = pickle.load(open('data/test_data_small_chunks_1_of_5.pickle', 'rb'))
...
for param in test_data:
    ...
    if n % 1000 == 0: # reset fs sum every 100 ts, output added FS
        i = int(n / 1000 - 1)
        # output current added FS
        dadi.Spectrum(neu_fs).to_file(f'data/dadi_test/small_chunks_1000/fs_{i}_syn')
        dadi.Spectrum(non_neu_fs).to_file(f'data/dadi_test/small_chunks_1000/fs_{i}_nonsyn')
        # reset fs sum
        neu_fs, non_neu_fs = np.zeros(neu_fs_small.shape), np.zeros(non_neu_fs_small.shape)
...
print(true)
pickle.dump(true, open('data/dadi_test/small_chunks_1000/true_small_chunks_1000_1_of_5.pickle','wb'))
